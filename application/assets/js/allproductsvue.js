new Vue({
    ...
    watch: {
        selectedCategory: async function(val) {
            ...
            this.filters.data.subcategories=[];
            this.filters.data.colors=[];
            if(!val) {
                await this.getFilters({
                    no_material: 1,
                    no_sizes: 1,
    ...
    computed: {
        selectedCategoryData() {
            if(this.selectedCategory) {
                return this.filters.data.categories.find(row => row.cat_slug == this.selectedCategory);
            }
            return null;
        },
        priceRangeWatcher() {
    ...
    methods: {
        async getFilters($args) {
            try {
                ...
                e.filters.data.range.min = res.data.range.min;
                e.filters.data.range.max = res.data.range.max;
        ...
        async getCategoriesFilter() {
            try {
                const e = this;
                e.categoriesLoading = true;
                const res = await api.get(`/public/filter/getCategories`);
                e.filters.data.categories = res.data;
                e.categoriesLoading = false;
            } catch($e) {
                e.categoriesLoading = false;
                return;
            }
