<table class="wp-list-table widefat fixed striped pages mt-3">
    ...
    <tbody v-if="!stockshape.loading && stockshape.data">
        <tr v-for="(ca, index) in stockshape.data" :key="`category-index-${index}`">
            <td>{{ ca.id }}</td>
            <td>{{ ca.hid }}</td>
            <td>{{ ca.title }}</td>
            <td>{{ ca.priority }}</td>
            <td>
                <div v-if="!inactive">
                    <a href="javascript:void(0)" @click.prevent="formInputs(true, {...ca, index, collection: inputJson(ca.collection, 'collection')}, `_id=${ca.hid}`)" class="button mb-1">Edit</a>
...
<p v-if="stockshape.loading" style="text-align:center;">Loading...</p>
<p v-if="!stockshape.loading && !stockshape.data.length" style="text-align:center;">
    There is no stock shape collection.
</p>
...
:page-range="10"
:click-handler="paginateStockshapeCollection"
