<table class="wp-list-table widefat fixed striped pages mt-3">
    ...
    <tbody v-if="!premiumBg.loading && premiumBg.data">
        <tr v-for="(ca, index) in premiumBg.data" :key="`category-index-${index}`">
            <td>{{ ca.id }}</td>
            <td>{{ ca.hid }}</td>
            <td>{{ ca.title }}</td>
            <td>{{ ca.priority }}</td>
...
<p v-if="premiumBg.loading" style="text-align:center;">Loading...</p>
<p v-if="!premiumBg.loading && !premiumBg.data.length" style="text-align:center;">
    There is no premium background collection.
</p>
...
:page-range="10"
:click-handler="paginatePremiumBGCollection"
